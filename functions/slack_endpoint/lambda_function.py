...
class BaseResponseHandler(object):
    ...
    def execute(self):
        ...
        ERROR_TEMPLATES = {
            ...
            'message_id_not_found': {'source': 'client', 'resp_template':'Error - The code you supplied is invalid'},
            'message_id_used': {'source': 'client', 'resp_template': 'Error - The code you suppied has already been used'},
            ...
            'response_delivery_timed_out': {'source': 'client', 'resp_template':'Error - the time window to respond to this message has expired'},
            'response_delivery_failed': {'source': 'server'},
            'message_status_update_failed': {'source': 'server'},
            NO_MESSAGE_ID: {'source': 'client', 'resp_template': 'Error - you did not supply a message code in your response. Please try again'},
            NO_RESPONSE: {'source': 'client', 'resp_template': 'Error - you did not supply a response after the message code. Please try again'},
        }
        try:
            ...
            if err_code in ERROR_TEMPLATES:
                ...
                if ERROR_TEMPLATES[err_code]['source'] == 'server':
                    resp_template = '*internal server error - {}. {}*'.format(err_code, HELP_TEXT)
                else:
                    resp_template = '*{}. {}*'.format(ERROR_TEMPLATES[err_code]['resp_template'],HELP_TEXT)
            else:
...
class SlashCommandHandler(BaseResponseHandler):
    ...
    def make_apig_response(self,resp_template='One moment please',fields={}):
        """
        """
        return {"statusCode":200, "body": json.dumps({"text": resp_template.format(**fields), "response_type": "in_channel"})}
...
class InteractiveMessageHandler(BaseResponseHandler):
    ...
    def make_apig_response(self,resp_template='You responded with *{}*',fields={}):
        ...
        original_attachment = copy.deepcopy(self.payload['original_message']['attachments'][0])
...
def lambda_handler(event, context):
    ...
    #TODO: Set SLACK_SIGNING_SECRET as environment variable in SSM
    #TODO: Set HELP_TEXT as environment variable
    # Validate that the message came from Slack
    slack_signing_secret = bytes(os.environ['SLACK_SIGNING_SECRET'],'utf8')
    timestamp = event['headers']['X-Slack-Request-Timestamp']
    if abs(time.time() - float(timestamp)) > 60 * 5:
        return {"statusCode": 200, "body": "invalid_auth_timestamp. {}".format(os.environ.get('HELP_TEXT','')) }

    sig_basestring = bytes('v0:{}:{}'.format(timestamp, event['body']),'utf8')
    my_signature = bytes('v0=' + hmac.new(slack_signing_secret,sig_basestring,hashlib.sha256).hexdigest(),'utf8')
    slack_signature = event['headers']['X-Slack-Signature'].encode()
    if not hmac.compare_digest(my_signature, slack_signature):
        return {"statusCode": 200, "body": "invalid_auth_signature. {}".format(os.environ.get('HELP_TEXT','')) }
    ...
    if 'type' in parsed_payload:
        ...
        if response_type == 'dialog_submission' or response_type == 'dialog_cancellation':
