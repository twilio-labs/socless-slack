...
from socless import socless_bootstrap, socless_dispatch_outbound_message, socless_template_string, init_human_interaction
...
def handle_state(context, target, target_type, message_template, receiver="", response_desc='[response]'):
    """
    ...
    extended_template = f"{message_template}\n```{SLACK_SLASH_COMMAND} {message_id} {response_desc}```\n"
    ...
    if not r.data['ok']:
        raise Exception(f"Human Reponse workflow failed to initiate, slack_client failed to send message: {r.data}")
